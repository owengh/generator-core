...
public class JavaFileMergerJaxp {
    ...
    /**
     * 
     * @param newFileSourceFullPath 文件全路径
     * @param existingFileFullPath  文件全路径
     * @return  两个文件全路径 合并
     * @throws FileNotFoundException
     */
    public String getNewJavaFileByAllFullPath(String newFileSourceFullPath, String existingFileFullPath) throws FileNotFoundException {
        CompilationUnit newCompilationUnit = JavaParser.parse(new File(newFileSourceFullPath));
        CompilationUnit existingCompilationUnit = JavaParser.parse(new File(existingFileFullPath));
        return mergerFile(newCompilationUnit,existingCompilationUnit);
    }
